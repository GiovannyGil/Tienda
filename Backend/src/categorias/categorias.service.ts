...
export class CategoriasService {
  ...
  async create(createCategoriaDto: CreateCategoriaDto): Promise<Categoria> {
    try {
      ...
      if (!categoria) throw new NotFoundException('No se pudo crear la categoria')
  ...
  async findAll():Promise<Categoria[]> {
    try {
      ...
      if (!categorias) throw new NotFoundException('No hay categorias registradas.')
  ...
  async update(id: number, updateCategoriaDto: UpdateCategoriaDto) {
    try {
      ...
      // si no encuentra nada, devolver un array vacio
      if (!categoriaUpdate) throw new BadRequestException('La categoria no existe')
      // devolver la categoria
  ...
  async softDelete(id: number) {
    try {
      ...
      if (!categoria) throw new BadRequestException('La categoria no existe o ya fue eliminada')
